...
rsync -ar -H --delete $SOURCE_DIR $DES_DIR/$TODAY/ && echo Backup $SOURCE complete `date` >> /var/log/backupd.log || echo Failed to backup $SOURCE directory on `date` >> /var/log/backupd.log
